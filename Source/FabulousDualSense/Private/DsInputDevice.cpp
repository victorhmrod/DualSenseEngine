...
void FDsInputDevice::SendControllerEvents()
{
	...
	for (auto ControllerId{0}; ControllerId < DsConstants::MaxDevicesCount; ControllerId++)
	{
		...
		{
			UE_LOG(LogDualSenseEngine, Warning, TEXT("Failed to read device input state: %s, Device: %s."),
		...
		if (ExtraStates[ControllerId].bOutputChanged)
		{
			...
			{
				UE_LOG(LogDualSenseEngine, Warning, TEXT("Failed to write device output state: %s, Device: %s."),
				       DsUtility::ReturnValueToString(ReadInputResult).GetData(), Context._internal.devicePath);
...
void FDsInputDevice::RefreshDevices()
{
	...
	switch (EnumDevicesResult)
	{
		...
		default:
			UE_LOG(LogDualSenseEngine, Warning, TEXT("Failed to enumerate devices: %s."),
...
{
	UE_LOG(LogDualSenseEngine, Log, TEXT("New device found: %s, Connection: %s."),
	...
	{
		UE_LOG(LogDualSenseEngine, Log, TEXT("Device connected: %s."), DeviceInfo._internal.path);
	...
	{
		UE_LOG(LogDualSenseEngine, Warning, TEXT("Failed to initialize device context: %s, Device: %s."),
                                      ...
                                      const FPlatformUserId PlatformUserId, const FInputDeviceId InputDeviceId)
{
	...
	UE_LOG(LogDualSenseEngine, Log, TEXT("Device disconnected: %s."), Context._internal.devicePath);
